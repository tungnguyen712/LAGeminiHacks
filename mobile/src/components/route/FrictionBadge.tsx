...
export const FrictionBadge = ({ level, size = 'md' }: FrictionBadgeProps) => {
  ...
  return (
    <View style={[styles.container, { backgroundColor: color + '15', borderColor: color + '30' }, size === 'sm' ? styles.sm : styles.md]}>
      <View style={[styles.dot, { backgroundColor: color }]} />
      <Text style={[styles.text, { color: color }, size === 'sm' ? styles.smText : styles.mdText]}>
        {label} Friction
      </Text>
    </View>
  );
